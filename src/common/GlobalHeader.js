    ...
    <>
        ...
        <ul className="navbar-nav mr-2">
          ...
          <ul className="dropdown-menu">
            ...
            <li><Link onClick={()=>{sessionStorage.clear(); window.location.reload(); window.location.href('/')}} style={{color: "red"}} className="dropdown-item" to="/">Log Out</Link></li>
          </ul>
        </div>
          ...
          {!sessionStorage?.username ? <i style={{fontSize: "30px"}} onClick={()=>{setShow(true)}} className="fa-regular fa-circle-user mr-1"></i> : <p className="m-0 rounded-circle d-flex justify-content-center align-items-center mr-2 mt-0" style={{width: "30px", height: "30px", border: location.pathname === '/' ? (scroll > 0 ? "2px solid black" : "2px solid white") : "2px solid black"}}>{sessionStorage?.username.slice(0,1)}</p>}
          </div>
